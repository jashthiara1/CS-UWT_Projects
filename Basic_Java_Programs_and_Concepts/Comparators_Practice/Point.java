...
import comparableandcomparators.CustomComparePoint;
import java.util.ArrayList;
import java.util.Collections;
import java.util.Objects;

/**
 * Comparators exercise.
 * 
 * @author Jthiara
 * @version FALL 2021
 */
public final class Point implements Comparable<Point> {
    ...
    public String toString() {
        final String tempVar = "%.2f";
        final StringBuilder builder = new StringBuilder();
        builder.append(getClass().getSimpleName());
        builder.append(" (");
        builder.append(String.format(tempVar, this.getX()));
        builder.append(", ");
        builder.append(String.format(tempVar, this.getY()));
        builder.append(")");
    ...
    public static void main(final String[] theArgs) {
        ...
        list.add(new Point(tempInt, 1));
       
        //Q1. Sort this list using natural ordering, i.e., using the in-built comparable interface which compares both x and y values
        Collections.sort(list);
        System.out.println(list);
        //Collections.sort(list, new CustomComparePoint());
        System.out.println(list);
        //Q2. Sort this list using custom ordering, i.e., using a custom comparator which compares only the y co-ordinate

    }

    @Override
    public int compareTo(final Point theO) {
        if (this.equals(theO)) {
            return 0;
        } else if (this.getX() > theO.getX() && this.getY() > theO.getY()) {
            return 1;
        } else {
            return -1;
        }
